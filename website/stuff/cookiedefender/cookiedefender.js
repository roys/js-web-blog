...
function GameOverState(renderer, changeStateCallback, extraArgs) {
    ...
    this.create = function () {
        ...
        if (this.wonGame) {
            ...
            if (this.playSounds) {
                this.audio = new Audio('/stuff/cookiedefender/game_lost.mp3');
                this.audio.load();
...
function PlayState(renderer, changeStateCallback, extraArgs) {
    ...
    this.create = function () {
        if (this.playSounds) {
            var audio = new Audio('/stuff/cookiedefender/alien_start.m4a');
            audio.volume = 0.05;
            audio.play();
        }
        this.renderer.clearScreen();
...
function Player(maxX, x, y) {
    ...
    this.isFiring = false;
    this.sounds = [];
    var sound = new Audio('/stuff/cookiedefender/player_shot.m4a');
    sound.load();
    sound.volume = 0.03;
...
function ExplosionGroup() {
    this.explosions = [];
    this.sounds = [];
    var sound = new Audio('/stuff/cookiedefender/explosion.m4a');
    sound.load();
    this.sounds[0] = sound.cloneNode(true);
